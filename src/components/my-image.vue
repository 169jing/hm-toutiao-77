...
export default {
  ...
  data () {
    return {
      ...
      // 封面图的地址
      //   v-lic 3.0 基于 webpack, 打包所有的资源，
      //   如果scr 绑定的数据，是不会去打包背后的资源的
      //   scr url 指定的资源， import资源 js 文件都可以打包
      //   主动通过 import导入地址，就会打包
      // 封面图的地址
      //   value: defaultImage
      defaultImage
    }
  ...
  methods: {
    ...
    openDialog () {
      ...
      // 打开对话框后 再获取素材列表数据  初始化获取会浪费资源
      this.getImages()
    },
    // 向后台请求素材列表提交的数据 并解构赋值给 data
    async getImages () {
      const {
        data: { data }
